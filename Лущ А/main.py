...
def load_students_from_file(filename: str):
    ...
    print(f"[INFO] Успішно завантажено {len(students)} студент(ів).")
    return students


def generate_student_report(students_list: list, output_filename: str, pass_threshold: int = 60):
    print(f"[INFO] Починаю генерацію звіту...")
    ...
    try:
        ...
        print(f"[INFO] Звіт успішно збережено у файл '{output_filename}'.")
